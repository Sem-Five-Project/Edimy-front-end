...
export const createFaq = async (faqDto: Omit<FaqDto, 'faqId' | 'createdAt'>): Promise<FaqDto> => {
    const response = await api.post('/faq', faqDto);
    return response.data;
};

export const updateFaq = async (id: number, faqDto: Omit<FaqDto, 'faqId' | 'createdAt'>): Promise<FaqDto> => {
