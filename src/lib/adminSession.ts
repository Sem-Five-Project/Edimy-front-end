...
export async function searchSessions(params: {
    ...
    page?: number;
    size?: number;
}): Promise<{ content: SessionsDto[]; totalElements: number; totalPages: number; }> {
    const response = await api.get(`${API_BASE}`, { params });
    return response.data;
}

export async function getOngoingSessions(page = 0, size = 10): Promise<{ content: SessionsDto[]; totalElements: number; totalPages: number; }> {
    const response = await api.get(`${API_BASE}/ongoing`, { params: { page, size } });
    return response.data;
}
...
export async function updateSession(id: number, sessionDto: SessionDto): Promise<SessionDto> {
