...
export interface ReVerificationDocument {
  id: string;
  type: 'Certificate' | 'Degree' | 'Professional License' | 'Experience Letter' | 'Portfolio' | 'Other';
  filename: string;
  uploadDate: string;
  fileSize: number;
  verified: boolean;
...
const mockReVerificationRequests: ReVerificationRequest[] = [
    ...
    newSubjectRequests: [
      {
        ...
        experienceLevel: 'Advanced',
        yearsOfExperience: 4,
        motivation: 'I have a strong background in physics and would like to expand my teaching portfolio to help more students.',
        qualifications: ['MSc in Physics', '4 years university teaching experience']
      }
    ],
    ...
    newSubjectRequests: [
      {
        ...
        experienceLevel: 'Expert',
        yearsOfExperience: 5,
        motivation: 'With my PhD in AI and 5 years of ML research, I want to share cutting-edge knowledge with students.',
        qualifications: ['PhD in Artificial Intelligence', 'Published ML research papers', '5 years industry ML experience']
      },
      {
        ...
        experienceLevel: 'Advanced',
        yearsOfExperience: 4,
        motivation: 'My experience in data analytics and research makes me qualified to teach comprehensive data science.',
        qualifications: ['Data Science certification', '4 years data analytics experience']
      }
    ],
    ...
    newSubjectRequests: [
      {
        ...
        yearsOfExperience: 3,
        motivation: 'Having lived in France for 3 years and obtained C2 proficiency, I can effectively teach French to students.',
        qualifications: ['DELF C2 Certificate', '3 years living in France', 'French literature studies']
      }
    ],
  ...
  },
  {
    ...
    newSubjectRequests: [
      {
        ...
        experienceLevel: 'Advanced',
        yearsOfExperience: 4,
        motivation: 'My background in biochemistry and molecular biology research qualifies me to teach biology effectively.',
        qualifications: ['PhD in Biochemistry', 'Published biology research', '4 years biology teaching']
      }
    ],
    ...
    reviewNotes: 'While qualifications are good, we require more direct biology teaching experience',
    supportingDocuments: [
      {
        id: 'REDOC006',
        type: 'Degree',
    ...
    newSubjectRequests: [
      {
        ...
        experienceLevel: 'Advanced',
        yearsOfExperience: 5,
        motivation: 'My MBA and 5 years of business consulting experience provide practical insights for business education.',
        qualifications: ['MBA from top university', '5 years business consulting', 'Business certification']
      },
      {
        subject: 'Accounting',
        proposedRate: 40,
        experienceLevel: 'Intermediate',
        yearsOfExperience: 3,
        motivation: 'My CPA certification and accounting practice experience make me qualified to teach accounting principles.',
...
const mockAuditLogs: ReVerificationAuditLog[] = [
  ...
  },
  {
    ...
    rejectionReason: 'Insufficient specialized biology teaching experience',
    notes: 'While qualifications are good, we require more direct biology teaching experience',
...
// API Functions
export async function getReVerificationRequests(params: GetReVerificationRequestsParams = {}): Promise<{ requests: ReVerificationRequest[]; total: number }> {
  // Fake delay
  ...
  if (params.submissionDateFrom) {
    filteredRequests = filteredRequests.filter(request =>
      new Date(request.submissionDate) >= new Date(params.submissionDateFrom!)
    );
  }
...
export async function getReVerificationRequestById(id: string): Promise<ReVerificationRequest | null> {
  // Fake delay
...
export async function approveReVerificationRequest(id: string, adminName: string, notes?: string): Promise<void> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 1000));
...
export async function rejectReVerificationRequest(id: string, adminName: string, reason: string, notes?: string): Promise<void> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 1000));
  ...
  console.log(`Re-verification request ${id} rejected by ${adminName}: ${reason}`);
}

export async function bulkApproveReVerificationRequests(ids: string[], adminName: string): Promise<void> {
  // Fake delay
  ...
  console.log(`Bulk approved ${ids.length} re-verification requests by ${adminName}`);
}

export async function bulkRejectReVerificationRequests(ids: string[], adminName: string, reason: string): Promise<void> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 1500));
  ...
  console.log(`Bulk rejected ${ids.length} re-verification requests by ${adminName}: ${reason}`);
}

export async function getSubjectsFromReVerificationRequests(): Promise<string[]> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 500));
...
export async function getReVerificationAuditLogs(requestId?: string): Promise<ReVerificationAuditLog[]> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  return mockAuditLogs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

export async function downloadReVerificationDocument(docId: string): Promise<string> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 500));
