...
describe('api helpers', () => {
  ...
  test('ratingAPI.submitRating returns response data on success', async () => {
    ...
    mockedAxios.create.mockReturnValue(mockedAxios);
    const payload = { sessionId: 1, tutorId: 't1', rating: 5, comment: 'Great' };
    mockedAxios.post.mockResolvedValue({ data: { success: true, data: { message: 'Submitted' } } });
  ...
  test('ratingAPI.submitRating returns fallback on error', async () => {
    ...
    expect(res).toEqual({ success: false, data: { message: 'Failed to submit rating' }, error: 'Network error' });
    expect(mockedAxios.post).toHaveBeenCalledWith('ratings', payload);
  });
});
