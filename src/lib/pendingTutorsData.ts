...
export interface TutorDocument {
  id: string;
  type: 'ID Proof' | 'Educational Certificate' | 'Professional Certificate' | 'Resume/CV' | 'Other';
  filename: string;
  uploadDate: string;
  fileSize: number;
  verified: boolean;
...
export interface ReviewAction {
  ...
  tutorId: string;
  adminName: string;
  action: 'Approved' | 'Rejected' | 'Requested Additional Info' | 'Under Review';
  date: string;
  notes?: string;
  rejectionReason?: string;
}
...
export async function getPendingTutors(filters?: {
  ...
  const allPendingTutors: PendingTutor[] = [
    {
      ...
      subjects: [
        { subject: 'Physics', proposedRate: 50, experienceLevel: 'Expert', yearsOfExperience: 8 },
        { subject: 'Mathematics', proposedRate: 45, experienceLevel: 'Advanced', yearsOfExperience: 5 }
      ],
      documents: [
        {
          id: 'DOC001',
      ...
      motivation: 'I am passionate about making complex physics concepts accessible to students. My research background in quantum mechanics and my teaching experience at the university level have prepared me to help students excel in their physics studies.',
      ...
      certifications: ['Certified Physics Teacher', 'Research Publication in Nature Physics'],
    ...
    },
    {
      ...
      subjects: [
        { subject: 'Computer Science', proposedRate: 55, experienceLevel: 'Expert', yearsOfExperience: 6 },
        { subject: 'Python Programming', proposedRate: 50, experienceLevel: 'Expert', yearsOfExperience: 6 },
        { subject: 'Web Development', proposedRate: 48, experienceLevel: 'Advanced', yearsOfExperience: 4 }
      ],
      documents: [
        {
          id: 'DOC004',
      ...
      motivation: 'After working in the tech industry for 6 years, I want to give back by helping students learn programming and computer science. I believe in hands-on learning and practical applications.',
      reviewNotes: 'Strong technical background, documents look good. Need to verify AWS certification.',
    ...
    },
    {
      ...
      subjects: [
        { subject: 'Spanish', proposedRate: 35, experienceLevel: 'Expert', yearsOfExperience: 12 },
        { subject: 'English Literature', proposedRate: 40, experienceLevel: 'Advanced', yearsOfExperience: 8 }
      ],
      documents: [
        {
          id: 'DOC007',
      ...
      motivation: 'I have been teaching Spanish and literature for over 12 years. My goal is to help students not just learn the language, but also appreciate the rich culture behind it.',
    ...
    },
    {
      ...
      subjects: [
        { subject: 'Chemistry', proposedRate: 52, experienceLevel: 'Expert', yearsOfExperience: 10 },
        { subject: 'Organic Chemistry', proposedRate: 58, experienceLevel: 'Expert', yearsOfExperience: 10 },
        { subject: 'Biochemistry', proposedRate: 55, experienceLevel: 'Advanced', yearsOfExperience: 7 }
      ],
      documents: [
        {
          id: 'DOC009',
      ...
      motivation: 'With over 10 years in academic research and teaching, I want to help students develop a strong foundation in chemistry and inspire them to pursue scientific careers.',
      hourlyRateRange: { min: 45, max: 65 },
      experience: '10 years university teaching, 15+ published research papers',
      education: 'PhD in Organic Chemistry from Harvard, MS in Chemistry from Caltech',
      certifications: ['American Chemical Society Member', 'Peer Reviewer for Chemistry Journals'],
      languages: ['English'],
    ...
    },
    {
      ...
      subjects: [
        { subject: 'Art', proposedRate: 32, experienceLevel: 'Advanced', yearsOfExperience: 5 },
        { subject: 'Digital Design', proposedRate: 45, experienceLevel: 'Expert', yearsOfExperience: 7 }
      ],
      documents: [
        {
          id: 'DOC013',
      ...
      motivation: 'Art has always been my passion, and I love sharing creative techniques with students. I specialize in helping students find their unique artistic voice while mastering fundamental skills.',
      ...
      certifications: ['Adobe Certified Expert', 'Youth Art Education Certificate'],
  ...
  if (filters?.search) {
    ...
    filteredTutors = filteredTutors.filter(tutor => 
      ...
      tutor.subjects.some(s => s.subject.toLowerCase().includes(searchLower))
    );
  }

  if (filters?.status && filters.status !== 'All') {
    filteredTutors = filteredTutors.filter(tutor => tutor.status === filters.status);
  }
  ...
  if (filters?.submissionDateFrom) {
    ...
    filteredTutors = filteredTutors.filter(tutor => 
      new Date(tutor.submissionDate) >= fromDate
    );
  }
  ...
  if (filters?.submissionDateTo) {
    ...
    filteredTutors = filteredTutors.filter(tutor => 
      new Date(tutor.submissionDate) <= toDate
    );
  }
...
export async function getPendingTutorById(id: string): Promise<PendingTutor | null> {
  // Fake delay
...
export async function approveTutor(tutorId: string, notes?: string): Promise<boolean> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 1000));
...
export async function rejectTutor(tutorId: string, reason: string, notes?: string): Promise<boolean> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 1000));
...
export async function requestAdditionalInfo(tutorId: string, requirements: string[]): Promise<boolean> {
  // Fake delay
...
export async function bulkRejectTutors(tutorIds: string[], reason: string): Promise<boolean> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 2000));
...
export async function getReviewHistory(tutorId: string): Promise<ReviewAction[]> {
  // Fake delay
  await new Promise((resolve) => setTimeout(resolve, 500));
