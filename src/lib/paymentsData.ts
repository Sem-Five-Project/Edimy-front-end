...
const generateDisputeId = () =>
  `DIS-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    payouts.push({
      ...
      tutor: {
        ...
        bankDetails: {
          ...
          bankName: ["Chase Bank", "Wells Fargo", "Bank of America"][
            Math.floor(Math.random() * 3)
          ],
        },
      },
...
export const generateMockDisputes = (count: number = 25): PaymentDispute[] => {
  ...
  const disputeTypes: PaymentDispute["disputeType"][] = [
    "refund_request",
    "service_issue",
    "billing_error",
    "fraud_claim",
    ...
    disputes.push({
      ...
      resolvedDate:
        Math.random() > 0.6
          ...
          : undefined,
      description:
        "Student reported issues with session quality and is requesting a refund.",
      evidence: {
...
export const processRefund = async (
  ...
  console.log(
    `Processed refund of $${amount} for transaction ${transactionId}: ${reason}`,
  );
  return true;
};
