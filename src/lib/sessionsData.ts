...
const generateMockSessions = (): SessionData[] => {
  ...
  const students: StudentInfo[] = [
    { id: 'std1', name: 'Alice Johnson', email: 'alice@example.com', joinedAt: '2024-01-15' },
    { id: 'std2', name: 'Bob Smith', email: 'bob@example.com', joinedAt: '2024-02-20' },
    { id: 'std3', name: 'Carol Williams', email: 'carol@example.com', joinedAt: '2024-03-10' },
    { id: 'std4', name: 'David Brown', email: 'david@example.com', joinedAt: '2024-01-25' },
    { id: 'std5', name: 'Eva Davis', email: 'eva@example.com', joinedAt: '2024-02-14' },
  ];

  const tutors: TutorInfo[] = [
    { id: 'tut1', name: 'Dr. Michael Chen', email: 'michael.chen@example.com', rating: 4.9, totalSessions: 156 },
    { id: 'tut2', name: 'Prof. Sarah Wilson', email: 'sarah.wilson@example.com', rating: 4.8, totalSessions: 203 },
    { id: 'tut3', name: 'Dr. James Rodriguez', email: 'james.rodriguez@example.com', rating: 4.7, totalSessions: 98 },
    { id: 'tut4', name: 'Ms. Emily Taylor', email: 'emily.taylor@example.com', rating: 4.9, totalSessions: 142 },
    { id: 'tut5', name: 'Dr. Robert Kim', email: 'robert.kim@example.com', rating: 4.6, totalSessions: 87 },
  ];
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const sessionFee = duration === 30 ? 25 : duration === 60 ? 45 : duration === 90 ? 65 : 85;

    const history: SessionHistory[] = [
      {
        action: 'Session Booked',
        timestamp: new Date(sessionDate.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    if (status === 'Cancelled') {
      history.push({
        ...
        timestamp: new Date(sessionDate.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    const attendance: AttendanceInfo = {
      ...
      studentJoinTime: status === 'Completed' || status === 'Ongoing' ? 
        new Date(sessionDate.getTime() + 2 * 60 * 1000).toISOString() : undefined,
      tutorJoinTime: status === 'Completed' || status === 'Ongoing' ? 
        sessionDate.toISOString() : undefined,
    ...
    sessions.push({
      ...
      payment: {
        ...
        transactionId: paymentStatus !== 'Pending' ? `txn_${Math.random().toString(36).substr(2, 9)}` : undefined,
        paidAt: paymentStatus === 'Paid' ? new Date(sessionDate.getTime() - 6 * 24 * 60 * 60 * 1000).toISOString() : undefined,
        refundedAt: paymentStatus === 'Refunded' ? new Date().toISOString() : undefined,
...
export const getSessions = async (filters: SessionFilters = {}): Promise<{
  ...
  if (filters.studentName) {
    filteredSessions = filteredSessions.filter(session =>
      session.student.name.toLowerCase().includes(filters.studentName!.toLowerCase())
    );
  }

  if (filters.tutorName) {
    filteredSessions = filteredSessions.filter(session =>
      session.tutor.name.toLowerCase().includes(filters.tutorName!.toLowerCase())
    );
  }
  ...
  if (filters.dateFrom) {
    filteredSessions = filteredSessions.filter(session =>
      session.scheduledDate >= filters.dateFrom!
    );
  }

  if (filters.dateTo) {
    filteredSessions = filteredSessions.filter(session =>
      session.scheduledDate <= filters.dateTo!
    );
  }
  ...
  const stats: SessionStats = {
    ...
    upcomingSessions: filteredSessions.filter(s => s.status === 'Upcoming').length,
    ongoingSessions: filteredSessions.filter(s => s.status === 'Ongoing').length,
...
export const cancelSession = async (id: string, reason: string, adminId: string): Promise<boolean> => {
...
export const exportSessionsData = async (filters: SessionFilters = {}): Promise<string> => {
  const { sessions } = await getSessions({ ...filters, limit: 1000 });
  
  // Generate CSV content
  const headers = [
    'Session ID', 'Student Name', 'Student Email', 'Tutor Name', 'Tutor Email',
    'Subject', 'Scheduled Date', 'Scheduled Time', 'Duration (min)', 'Status',
    'Session Fee', 'Payment Status', 'Student Joined', 'Tutor Joined', 'Created At'
  ];
...
export const getSessionAnalytics = async (): Promise<{
  ...
  monthlyStats: { month: string; completed: number; cancelled: number; revenue: number }[];
}> => {
  await new Promise(resolve => setTimeout(resolve, 500));

  // Subject trends
  const subjectCounts = mockSessions.reduce((acc, session) => {
  ...
  // Busy hours
  const hourCounts = mockSessions.reduce((acc, session) => {
