...
//     useState<ExtendedFilterOptions>(filters);
...
//           Object.entries(effectiveFilters.timePeriods).forEach(
//             ([weekday, periods]) => {
...
//           selectedDate: effectiveFilters.selectedDate
//             ?.toISOString()
//             .split("T")[0],
...
//           setTotalResults(response.data?.totalElements ?? normalized.length);
//         } else {
//           setTutors([]);
//           setTotalPages(1);
//           setTotalResults(0);
...
//       }
//     },
//     [appliedFilters, appliedSearchTerm, itemsPerPage, buildEffectiveFilters],
//   );
...
//     switch (val) {
//       case "grade-1-5":
//         return "PRIMARY/GRADE 1-5";
//       case "grade-6-11":
//         return "SECONDARY/GRADE 6-11";
//       case "ordinary-level":
//         return "HIGHSCHOOL/ADVANCED_LEVEL"; // adjust if different
//       case "advanced-level":
...
//         session = {
//           date: f.selectedDate
//             ? f.selectedDate.toISOString().split("T")[0]
//             : null,
...
//           subjectName: subjName,
//           hourlyRate:
//             s.hourly_rate ?? s.hourlyRate ?? t.hourlyRate ?? t.hourly_rate ?? 0,
//         } as Subject;
//       });
//     }
...
//     const primaryRate =
//       subjects[0]?.hourlyRate ?? t.hourlyRate ?? t.hourly_rate ?? 0;
...
//         t.experienceMonths ?? t.experience_months ?? t.experience ?? 0,
//       ),
//       subjects,
...
//       user &&
//       user.role === "STUDENT" &&
//       (user.educationLevel === undefined || user.stream === undefined)
//     ) {
...
//               Connect with experienced educators tailored to your needs
//             </p>
//           </div>
...
//           <div
//             className={`border-t border-white/20 transition-all duration-500 ease-in-out overflow-hidden ${
//               isFilterOpen
...
//                 {appliedFilters.classType && (
//                   <Badge className="bg-white/20 text-white border-white/30">
//                     {appliedFilters.classType === "one-time"
//                       ? "One-time"
//                       : "Monthly"}
//                   </Badge>
//                 )}
...
useEffect(() => {
  ...
  const fetchTutorsBasedOnAcademicInfo = async () => {
    // Guard clause: Don't run if there's no user, the user is not a student,
    // or if the necessary academic info isn't available yet.
    if (!user || user.role !== 'STUDENT' || user.educationLevel === null || user.stream === null) {
      // If the info is missing, it implies another effect or process is loading it.
      // We'll wait for the next render when `user` is updated.
      return;
    }
