...
const SUBJECTS = {
  'advanced-level': {
    ...
    'technology': ['Engineering Technology', 'Science for Technology', 'Mathematics']
  },
  'ordinary-level': ['Mathematics', 'Science', 'English', 'Sinhala', 'Tamil', 'History', 'Geography'],
  'grade6-9': ['Mathematics', 'Science', 'English', 'Environmental Studies', 'Sinhala', 'Tamil'],
  'grade1-5': ['Mathematics', 'Science', 'English', 'Environmental Studies', 'Sinhala', 'Tamil']
};
  ...
  useEffect(() => {
    ...
    if (filters.educationLevel) {
      if (filters.educationLevel === 'advanced-level' && filters.stream) {
        subjects = SUBJECTS['advanced-level'][filters.stream as keyof typeof SUBJECTS['advanced-level']] || [];
      } else {
        subjects = SUBJECTS[filters.educationLevel as keyof typeof SUBJECTS] as string[] || [];
      }
    }
    ...
    if (subjects.length > 0) {
      const validSubjects = filters.subjects.filter(subject => subjects.includes(subject));
      if (validSubjects.length !== filters.subjects.length) {
        onFilterChange('subjects', validSubjects);
      }
    }
  }, [filters.educationLevel, filters.stream, filters.subjects, onFilterChange]);
            ...
            <div className="max-h-48 overflow-y-auto border-2 border-gray-200 rounded-lg p-4 bg-gray-50">
              <div className="space-y-3">
                {availableSubjects.map((subject) => (
                  <div key={subject} className="flex items-center space-x-3 p-2 hover:bg-white rounded-lg transition-colors">
                    <Checkbox
                      id={subject}
