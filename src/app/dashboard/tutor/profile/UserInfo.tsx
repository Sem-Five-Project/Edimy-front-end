...
const UserInfo: React.FC<UserInfoProps> = ({ user }) => {
  ...
  const [ratingsData, setRatingsData] = useState<{ ratings: any[]; total: number; page: number; limit: number }>({
    ratings: [],
    total: 0,
    page: 1,
    limit: 5,
      ...
      if (user?.id) {
        try {
          ...
          if (Array.isArray(responseData)) {
            ...
            const sum = arr.reduce((s, it) => s + (Number(it.ratingValue ?? it.rating ?? it.score ?? 0) || 0), 0);
          ...
          } else if (responseData?.averageRating !== undefined || responseData?.totalRatings !== undefined) {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (Array.isArray(dataAny)) {
          ...
          rawList = dataAny?.ratings ?? dataAny?.results ?? (Array.isArray(dataAny) ? dataAny : []);
        ...
        const ratings = (rawList ?? []).map((r: any) => ({
          id: r.ratingId ?? r.id,
          reviewerName: r.studentName ?? r.reviewerName ?? r.userName ?? r.user ?? 'Anonymous',
          ...
          <div className="flex items-center space-x-4 mt-3">
            <div className="flex items-center bg-white/20 rounded-lg px-3 py-1">
              ...
              <span className="text-sm font-medium">
                {ratingSummary.averageRating ? ratingSummary.averageRating.toFixed(1) : "-"} Rating
              </span>
            </div>
            <div className="flex items-center bg-white/20 rounded-lg px-3 py-1">
            ...
            {ratingsData.ratings.map((r, idx) => (
              <div key={r.id ?? idx} className="border rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div className="font-medium">{r.reviewerName ?? r.userName ?? r.user ?? 'Anonymous'}</div>
                  <div className="text-sm text-yellow-500">{r.rating ?? r.score ?? '-'} ⭐</div>
                </div>
                <div className="text-sm text-gray-600 mt-1">{r.comment ?? r.review ?? ''}</div>
                <div className="text-xs text-gray-400 mt-2">{new Date(r.createdAt ?? r.time ?? Date.now()).toLocaleString()}</div>
              </div>
            ))}

            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">Total: {ratingsData.total}</div>
              <div className="space-x-2">
                ...
                <button
                  onClick={onNext}
                  disabled={ratingsData.page >= Math.ceil((ratingsData.total || 1) / ratingsData.limit)}
                  className="px-3 py-1 rounded bg-gray-100 disabled:opacity-50"
                >
                  Next
                </button>
