...
import { cancelSession, type SessionData } from "@/lib/sessionsData";
...
export default function OngoingSessionsPage() {
  ...
  const [selectedSession, setSelectedSession] =
    useState<OngoingSessionWithDetails | null>(null);
  ...
  const [viewSession, setViewSession] = useState<
    null | import("@/lib/adminSession").SessionDto
  >(null);
  ...
  const handleJoinSession = async (
    session: OngoingSessionWithDetails,
    mode: JoinMode,
  ) => {
    ...
    try {
      ...
      // Add audit log entry
      const action =
        mode === "observer" ? "joined as Observer" : "joined as Participant";
  ...
  const getAttendanceStatusColor = (
    status: OngoingSessionWithDetails["attendanceStatus"],
  ) => {
    switch (status) {
      case "both-joined":
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Ongoing Sessions ({ongoingSessions.length} total)
          </CardTitle>
        ...
        <DialogContent className="bg-gray-900 text-white">
          ...
          {viewLoading ? (
            ...
            <div className="space-y-2">
              <div>
                <b>Session ID:</b> {viewSession.sessionId}
              </div>
              <div>
                <b>Title:</b> {viewSession.title}
              </div>
              <div>
                <b>Tutor ID:</b> {viewSession.tutorId}
              </div>
              <div>
                <b>Class ID:</b> {viewSession.classId}
              </div>
              <div>
                <b>Start Time:</b>{" "}
                {new Date(viewSession.startTime).toLocaleString()}
              </div>
              <div>
                <b>End Time:</b>{" "}
                {new Date(viewSession.endTime).toLocaleString()}
              </div>
              <div className="flex gap-2 mt-4">
                <Button
                  variant="default"
                  className="px-4 py-2 rounded font-semibold border border-white"
                  onClick={() => handleJoinMeeting(viewSession.moderatorLink)}
                >
                  Join as Host
                </Button>
                <Button
                  variant="secondary"
                  className="px-4 py-2 rounded font-semibold border border-white"
                  onClick={() => handleJoinMeeting(viewSession.participantLink)}
                >
                  Join as Participant
                </Button>
              </div>
