...
const profileSectionTitleVariants = cva("font-semibold tracking-tight transition-colors", {
  variants: {
    size: {
      sm: "text-sm",
      default: "text-base",
...
const ProfileSection = React.forwardRef<HTMLDivElement, ProfileSectionProps>(
  ({ className, variant, title, titleVariant, titleSize, children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn(profileSectionVariants({ variant, className }))} {...props}>
        {title && (
          <h3 className={cn(profileSectionTitleVariants({ variant: titleVariant, size: titleSize }))}>{title}</h3>
        )}
        {children}
      </div>
    )
