...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>((props: any, ref) => {
  ...
  if (!active || !_payload.length) return null;

  const tooltipLabel = (() => {
    if (hideLabel || !_payload.length) return null;
    const item = _payload[0];
    ...
    if (labelFormatter) return <div className={cn('font-medium', labelClassName)}>{labelFormatter(value, _payload)}</div>;
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, any>(({ className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey }, ref) => {
  ...
  const _payload: any[] = payload ?? [];
  if (!_payload.length) return null;
