...
import { Star, Clock, Users, GraduationCap, MapPin } from "lucide-react";
...
export function TutorInfoCard({ tutor, className = "", experienceFormatter = (months: number) => {
  ...
  const minRate = Math.min(...(tutor.subjects?.map((s) => s.hourlyRate) || [0]));
    ...
    <Card className={`relative border-0 overflow-hidden rounded-2xl shadow-xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-md ${className}`}>
      ...
      <CardContent className="relative p-6 pl-8 space-y-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6">
          {/* Enhanced Avatar - Removed Green Dot */}
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {tutor.rating?.toFixed(1) || "New"} ({tutor.reviews || 0} reviews)
                </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50/50 dark:bg-gray-900/20 rounded-xl p-4">
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 mb-1">
                  <GraduationCap className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                  <span className="font-semibold text-gray-900 dark:text-white">{experienceFormatter(tutor.experience ?? 0)}</span>
                </div>
            ...
            {/* Subjects with Rates - Enhanced Cards */}
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {tutor.subjects?.map((subject, index) => (
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400 italic">
                    Subjects will appear based on selection
                  </div>
                )}
              </div>
            </div>
            ...
            {tutor.languages && tutor.languages.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tutor.languages.slice(0, 3).map((language) => (
                  <Badge
                ...
                {tutor.languages.length > 3 && (
                  <Badge variant="secondary" className="text-xs">+{tutor.languages.length - 3}</Badge>
                )}
              </div>
            )}
          </div>
