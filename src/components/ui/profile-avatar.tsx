  ...
  "relative inline-flex items-center justify-center overflow-hidden rounded-full bg-muted transition-all duration-200",
  {
    variants: {
      ...
      variant: {
        ...
        gradient:
          "border-0 ring-4 ring-gradient-to-r from-primary/30 via-accent/30 to-secondary/30",
...
const profileAvatarImageVariants = cva(
...
const ProfileAvatar = React.forwardRef<HTMLDivElement, ProfileAvatarProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(profileAvatarVariants({ size, variant, className }))}
        {...props}
      >
        {src ? (
          <img
