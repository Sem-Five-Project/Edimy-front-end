...
export default function MonthlyClassBooking({
  ...
  const generateMonthlySlots = useCallback(
    (patterns: SelectedSlotPattern[]): OccurrenceSlot[] => {
    ...
    try {
      ...
      if (response.success && response.data.success) {
        ...
        if (response.data.failedSlots && response.data.failedSlots.length > 0) {
          const failedTimes = response.data.failedSlots
            .map(
              (slot: { dayOfWeek: number; time: string; reason: string }) =>
                `${DAYS_OF_WEEK.find((d) => d.value === slot.dayOfWeek)?.label} ${slot.time}`,
            )
            .join(", ");
          setError(
            `The following slots are no longer available: ${failedTimes}`,
          );
        } else {
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200">
        <CardHeader>
          ...
          <p className="text-sm text-blue-700">
            Select up to 4 weekly time patterns. Classes will automatically
            repeat every week until month end.
          </p>
        </CardHeader>
      </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center">
            ...
            >
              {isLoading
                ? "Booking..."
                : `Confirm Booking - LKR ${totalCost.toLocaleString()}`}
            </Button>
          </div>
        </div>
      )}
